...
$customTriplets = if ($null -ne $pkg.customTriplets) { @($pkg.customTriplets) } else { @() }
./invoke-build.ps1 -PackageName $PackageName -PackageAndFeatures $pkg.package -CustomTriplets $customTriplets -LinkType $pkg.linkType -BuildType $pkg.buildType -StagedArtifactsPath $StagedArtifactsPath -VcpkgHash $pkg.vcpkgHash -Publish $pkg.publish -ShowDebug:$ShowDebug
