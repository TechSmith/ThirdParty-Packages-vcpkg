...
if (Test-Path $portDir) {
    Copy-Item "$PSScriptRoot/aom/1001-tsc-aom-fix-nasm-multipass-check.patch" "$portDir/fix-nasm-multipass-check.patch"
    $patchSuccess = Apply-VcpkgPortPatch -PortName "aom" -PatchFile "$PSScriptRoot/aom/patch-aom-portfile.patch"
    if (-not $patchSuccess) {
