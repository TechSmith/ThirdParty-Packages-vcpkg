...
if (Test-Path $pathToWhisperCliExe) {
    ...
    # Parse features from PackageAndFeatures string
    $features = Get-Features -packageAndFeatures $PackageAndFeatures
    $avx2Enabled = $features -contains "avx2"
    
    $testScripts = @()
    
    # Always run transcription test
    $testScripts += @{
        Script = "test001--verify-transcription.ps1"
        Args = $testScriptArgs
    }
    
    # Always run transcription speed benchmark
    $testScripts += @{
        Script = "test002--transcription-speed.ps1"
        Args = $testScriptArgs
    }
    
    # Add Windows-specific tests
    if((Get-IsOnWindowsOS)) {
        # AVX-512 verification test
        $testScripts += @{
            Script = "test003--verify-no-avx512.ps1"
            Args = $testScriptArgs
        }
        
        # AVX2 verification test
        if ($avx2Enabled) {
            Write-Message "> AVX2 feature detected - will verify AVX2 is enabled"
        } else {
            Write-Message "> AVX2 feature NOT detected - will verify AVX2 is disabled"
        }
        
        $avx2TestArgs = $testScriptArgs.Clone()
        # Pass boolean as string "1" or "0" since Invoke-Powershell has type inference issues
        $avx2TestArgs.ShouldBeEnabled = if ($avx2Enabled) { "1" } else { "0" }
        
        $testScripts += @{
            Script = "test004--verify-avx2-status.ps1"
            Args = $avx2TestArgs
        }
    }
    
    foreach($test in $testScripts) {
        Write-Message "$(NL)Running tests: $($test.Script)..."
        Invoke-Powershell -FilePath "$($test.Script)" -ArgumentList $test.Args
